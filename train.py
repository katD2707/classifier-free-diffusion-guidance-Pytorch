...
def train(params: argparse.Namespace):
    assert (
        params.genbatch % (torch.cuda.device_count() * params.clsnum) == 0
    ), "please re-set your genbatch!!!"
    # initialize settings
    ...
    # load data
    params.chmul = [i for i in params.chmul if i.isdigit()]
    ...
    dataloader, sampler = load_data(
        ...
        input_channels=params.inch,
        image_size=imagesize,
        mode=["train", "val"],
        class_cond=True,
        transforms=transforms,
        batch_size=params.batchsize,
        n_workers=params.numworkers,
    )
    # initialize models
    net = Unet(
        in_ch=params.inch,
    ...
    if os.path.exists(lastpath):
        ...
        checkpoint = torch.load(
            os.path.join(params.moddir, f"ckpt_{lastepc}_checkpoint.pt"),
            map_location="cpu",
        )
    ...
    # DDP settings
    diffusion.model = DDP(
        diffusion.model, device_ids=[local_rank], output_device=local_rank
    )
    ...
    cosineScheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer=optimizer, T_max=params.epoch, eta_min=0, last_epoch=-1
    )
    ...
    for epc in range(lastepc, params.epoch):
        ...
        if (epc + 1) % params.interval == 0:
            ...
            with torch.no_grad():
                lab = torch.ones(
                    params.clsnum, each_device_batch // params.clsnum
                ).type(torch.long) * torch.arange(start=0, end=params.clsnum).reshape(
                    -1, 1
                )
                lab = lab.reshape(-1, 1).squeeze()
                lab = lab.to(device)
                cemb = cemblayer(lab)
                genshape = (each_device_batch, params.inch, imagesize, imagesize)
                if params.ddim:
                ...
                samples = torch.concat(all_samples, dim=1).reshape(
                    params.genbatch, params.inch, imagesize, imagesize
                )
                if local_rank == 0:
                    save_image(
                        samples,
                        os.path.join(params.samdir, f"generated_{epc+1}_pict.png"),
...
def main():
    ...
    parser.add_argument(
        "--numworkers", type=int, default=4, help="num workers for training Unet model"
    )
    parser.add_argument(
        "--inch", type=int, default=3, help="input channels for Unet model"
    )
    parser.add_argument(
        "--modch", type=int, default=64, help="model channels for Unet model"
    )
    parser.add_argument("--T", type=int, default=1000, help="timesteps for Unet model")
    parser.add_argument(
        "--outch", type=int, default=3, help="output channels for Unet model"
    )
    parser.add_argument(
        "--chmul",
        type=list,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="number of resblocks for each block in Unet model",
    )
    parser.add_argument(
        "--cdim", type=int, default=10, help="dimension of conditional embedding"
    )
    parser.add_argument(
        "--useconv",
        type=bool,
        default=True,
        help="whether use convlution in downsample",
    )
    parser.add_argument(
        "--droprate", type=float, default=0.1, help="dropout rate for model"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="epoch interval between two evaluations",
    )
    parser.add_argument("--data", type=str, default="mnist", help="data name")
    parser.add_argument("--datadir", type=str, default=None, help="data addresses")
